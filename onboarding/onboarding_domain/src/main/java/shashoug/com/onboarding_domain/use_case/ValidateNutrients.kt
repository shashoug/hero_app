package shashoug.com.onboarding_domain.use_case
...
class ValidateNutrients {
    ...
    operator  fun invoke(
        ...
        if(carbsRatio == null || proteinRatio == null || fatRatio == null){
            return  Result.Error(
                message = UiText.DynamicString("Please enter valid values")
            )
        }
        if(carbsRatio + proteinRatio + fatRatio != 100){
            return Result.Error(
                message = UiText.DynamicString("Please make sure your entries covers the 100%")
            )
        }
